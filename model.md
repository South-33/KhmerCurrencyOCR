...
| 2026-06-05 | synthetic | keep | `webgl_fan_fullschema_v1` has strong fan/fragment pressure but no same-denomination repeats (`max_same_class_per_image=1` across 64 images). Added `webgl_same_class_repeat_fan_v1` plus `check_webgl_count_stress.py`; its 4-image diagnostic smoke passed with 22 physical targets, 34 kept fragments, all 4 images containing same-class repeats, max same-class count 3, and parent-fused all-fragment counts matching physical targets. Keep diagnostic until broader sequence coverage and real repeat-fan validation. |
...
| `webgl_same_class_repeat_fan_v1` | `data/synthetic/cashsnap_webgl_same_class_repeat_fan_smoke_v1` | detect, fragment diagnostic | Diagnostic count-stress smoke: 4 fan images at 960x720 `visual_scale: 1` produced 22 physical targets, 34 kept fragments, all 4 images with same-class repeats, max same-class count 3, and parent-fused all-fragment counts matching physical targets. | Proves same-denomination fan repeat generation works. Not a trainable candidate until broader sequence distribution and real repeated-fan validation pass. |
