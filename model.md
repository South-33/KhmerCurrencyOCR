...
2. Repair the real curriculum before more synthetic scale: keep p24/p48-style caps, add rare-aware sampling or weighting instead of just raising common-class/background quotas, and require any expanded real-only control to beat the p24 gate without eroding `KHR_50000`. Keep `configs/cashsnap_v1_plus_webgl_accepted_nowarmup_probe.yaml` as bounded ablation evidence only.
...
- `configs/cashsnap_v1_real_only_p48_bg48_seed.yaml` and `configs/cashsnap_v1_real_only_p96_bg96_seed.yaml` prove capped real-only expansion is safer than full all-target but still not enough: p48/bg48 tests at `0.833190`, p96/bg96 at `0.819153`, while the p24 real-only gate remains `0.835861`.
