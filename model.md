...
Step-back synthetic diagnosis: the accepted blend passes the loose `accepted_blend_v1` domain-gap gate but fails the geometry-aware `accepted_blend_geometry_v1` gate. Synthetic visible boxes are much smaller than real train boxes on the accepted mix (`area/width/height` means `0.083/0.309/0.245` synthetic vs `0.456/0.726/0.626` real), and `KHR_50000` still has a scale gap (`area 0.136` synthetic vs `0.462` real). The new `webgl_real_scale_stack_geometry_v1` diagnostic proves the renderer can hit aggregate real-scale stack geometry (`0.309/0.612/0.470`, deltas `-0.147/-0.115/-0.156`) under closeup framing. A larger 40-image pool confirms per-class aspect drift persists, led by `USD_20` (`+1.076`), `USD_10` (`+0.586`), and `KHR_10000` (`-0.543`), while `KHR_50000` aspect is no longer the geometry blocker (`-0.035`). The next generator-side repair should target class-shape/pose diversity and per-class geometry before another model training spend.
...
- `check_synthetic_dataset_scorecard.py` turns the dataset-perfection rubric into a thin executable summary over readiness/domain-gap artifacts, geometry-domain-gap artifacts, mined-real review/quality artifacts, split class coverage, and mined held-out diagnostic utility comparisons. Current scorecard is blocked: synthetic coverage, package trust, and loose accepted-blend domain gap pass; real anchor/holdout, clean-real rare train coverage, hard negatives, visible-note geometry, diagnostic real utility, and protected real utility are blocked; mined edge-case inventory has 17 diagnostic-ready scoreable image(s) / 35 boxes, not protected transfer proof.
...
- Recipe diagnostic gates now support a `domain_gap` gate that composes a bounded real-control train list with a packaged WebGL root and runs `audit_yolo_domain_gap.py`; `webgl_real_scale_stack_geometry_v1` uses it to prove closeup-framed stack renders pass aggregate visible-note geometry before any model probe.
...
| 2026-06-05 | gates | keep | `check_synthetic_dataset_scorecard.py` now summarizes the PDF rubric over existing artifacts. Latest scorecard is `blocked` with `3` pass / `2` review / `6` blocked axes: pass for target-condition synthetic coverage, package label trust, and loose accepted-blend domain gap; blocked for real anchor/holdout, clean-real rare train coverage, hard negatives, visible-note geometry, diagnostic real utility, and protected real utility; review for mined edge-case inventory and release-grade governance/provenance. |
