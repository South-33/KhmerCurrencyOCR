...
- `build_webgl_staged_rare_dose_configs.py` builds accepted-blend + rare-support staged dose configs/lists and supports `--stem-prefix` so alternate dose roots do not overwrite each other. Fan and stack dose 1/2 pass the accepted-blend domain-gap preset; dose 4/8/16 are written for audit context but are domain-flagged before training.
