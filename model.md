...
- WebGL generic clean/stack/fan recipes support `--class-sequence` for targeted weak-denomination smoke/dose tests; `webgl_rare_class_support_v1` has a tiny diagnostic proof, not trainable-candidate approval.
...
| `webgl_rare_class_support_v1` | `data/synthetic/cashsnap_webgl_rare_class_support_recipe_smoke` | detect, fragment diagnostic | Diagnostic tiny smoke only: catalog-driven fan render at 960x720 `visual_scale: 1` produced 1 image / 5 boxes with one each for `KHR_2000`, `KHR_50000`, `KHR_20000`, `KHR_10000`, and `KHR_5000`; label-view QA passed with 9 fragments and 5 physical count targets. | Proves class-sequence rare-KHR control works. Not a trainable candidate until class-distribution audit and real rare-class validation pass. |
