...
- HDRI/equirectangular environment-map import for realistic lighting/reflection variation; prefer CC0 sources such as Poly Haven and record exact asset/license metadata before using them.
...
| 2026-06-05 | gates | keep | The WebGL appearance-diversity gate now also measures focus-blur range, key-light color count, and hemi/key intensity ranges. The 7-root trainable-candidate suite still passes with 304 images; current WebGL nuisance spread is strong for label-safe camera/lighting/postprocess variation, while HDRI environment maps, exact lens distortion, and true depth blur remain future renderer work. |
