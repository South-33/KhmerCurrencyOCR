...
Current evidence gap: P1 transfer still has 0 protected/promoted real fan/overlap stress labels. Existing `cashsnap_v1` mined review labels now provide 5 diagnostic-ready train-split stress images / 17 boxes, but they are not protected transfer proof and must not replace rights-clean holdout captures.
...
- `check_synthetic_dataset_scorecard.py` turns the dataset-perfection rubric into a thin executable summary over readiness/domain-gap artifacts plus mined-real review/quality artifacts. Current scorecard is blocked: synthetic coverage, package trust, and accepted-blend domain gap pass; real anchor/holdout, hard negatives, and real utility are blocked; mined edge-case inventory has 5 diagnostic-ready scoreable image(s) / 17 boxes, not protected transfer proof.
...
- Mined real diagnostic YOLO dataset: `rl python scripts\build_mined_real_scoreable_dataset.py --min-images 5 --min-stress-images 5 --min-boxes 17`
...
| 2026-06-05 | real bridge | keep | Mined real review quality is now executable and partly audited. `manifests/mined_real_benchmark_review_quality.csv` marks 5 diagnostic-ready stress image(s) / 17 boxes (`3` KHR 5000 dense-overlap train images plus `2` same-denomination train images) and leaves 15 image(s) / 30 boxes as `needs_review`. This improves the bridge but is not protected transfer proof because source split separation still matters. |
| 2026-06-05 | evaluation | note | The mined diagnostic YOLO scoreboard is executable: `build_mined_real_scoreable_dataset.py` builds 5 train-split images / 17 boxes (`KHR_5000` 9, `KHR_1000` 4, `USD_20` 4), and `check_yolo_dataset.py` passes. On this tiny diagnostic slice, clean checkpoint mAP50-95 is `0.606550`; p24 real-only scores `0.555784`/`0.510973` for seeds 0/1; accepted WebGL blend scores `0.573714`/`0.563220`, beating p24 by `+0.017930`/`+0.052247` but still below clean by `-0.032836`/`-0.043330`. Treat as a stress smoke, not synthetic promotion evidence. |
