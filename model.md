...
Latest model comparison: the old 621-image accepted-WebGL blend remains rejected (`0.810` held-out clean test mAP50-95 vs `0.884` clean checkpoint). A stricter 445-image blend of individually accepted WebGL recipes passes the matched real-only clean-test gate (`0.836855` vs `0.835861`, delta `+0.000994`), but fails the per-class guard on `KHR_2000` (`-0.052433`). Whole-recipe leave-one-out probes did not fix it. The first rare-support staged dose did: adding 2 catalog-balanced rare-KHR fan images to the accepted blend tests at `0.838473`, beating matched p24 real-only by `+0.002612` with no per-class failures (`KHR_2000 -0.027228`, worst `KHR_50000 -0.043206`) and beating the full accepted blend by `+0.001618`. Dose 1 is rejected (`0.825533`, `-0.010328` vs p24, `KHR_50000 -0.143900`), while dose 4/8/16 should not be trained yet because the accepted-blend domain-gap preset fails at dose 4 on `KHR_20000` synthetic/real box ratio. Full all-target seeds regress badly: uncapped accepted synthetic tests at `0.807251` vs `0.883801` clean checkpoint (delta `-0.076550`), capped accepted synthetic tests at `0.785412` (delta `-0.098389`), and full-real real-only tests at `0.783482` (delta `-0.100319`). Treat dose 2 as a promising bounded ablation, not promotion; real fan/overlap transfer proof is still missing.
...
- `build_webgl_staged_rare_dose_configs.py` builds accepted-blend + rare-support staged dose configs/lists. Dose 1 and dose 2 pass the accepted-blend domain-gap preset; dose 4/8/16 are written for audit context but are domain-flagged before training.
...
| 2026-06-05 | evaluation | provisional | Rare-support staged dose 2 is the first positive accepted-blend shaping probe: `configs/webgl_staged_dose/cashsnap_v1_plus_webgl_accepted_rare_support_dose2.yaml` trained 1 epoch from the clean checkpoint and tested at `0.838473` mAP50-95, `+0.002612` vs matched p24 real-only with no per-class failures (`KHR_2000 -0.027228`, worst `KHR_50000 -0.043206`) and `+0.001618` vs the full accepted blend. Dose 1 is rejected (`0.825533`, `-0.010328` vs p24, `KHR_50000 -0.143900`). Treat dose 2 as bounded ablation evidence, not promotion, until repeated or real-stress validated. |
