...
| 2026-06-05 | validation | keep | `prepare_mined_real_benchmark_review.py` now builds a separate mined-real review package from those queues. Default stress-first, unique-origin run selected 20 candidates / 47 draft boxes across KHR_5000 overlap, simple overlap, same-denomination fan, thin KHR_5000/KHR_20000, and weak KHR_20000/KHR_50000; custom `check_real_fan_benchmark.py` validation passed with all rows draft-only. Visual samples show useful real phone/table hand-overlap anchors, but draft boxes need visible-region/quality review and these existing `cashsnap_v1` images still do not count as protected real-transfer proof. |
