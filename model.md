...
- `check_mined_real_review_quality.py` validates mined review quality rows against draft YOLO labels, counts ready scoreable/stress images, and only materializes `.scoreable.txt` labels after every box in an image is decided.
...
- Mined real review quality bridge: `rl python scripts\check_mined_real_review_quality.py`
...
| 2026-06-05 | real bridge | keep | Mined real review quality is now executable instead of implicit. The current 20-image draft package has 47 draft boxes, but every row is still `needs_review`; scorecard evidence remains 0 ready scoreable image(s), 0 ready stress image(s), and 0 scoreable boxes until visual review promotes rows as `clear`/`partial_clear`. |
