...
A third focused Roboflow search on 2026-05-27 found a stronger lead: [cuurecy-detection-is](https://universe.roboflow.com/ddd8889/cuurecy-detection-is), a CC BY 4.0 instance-segmentation dataset with 2.3k images and KHR/USD front/back classes such as `10000-riel-f`, `20000-riel-b`, `5-US-f`, and `100-us-b`. It was exported locally in YOLOv8 segmentation format, and `scripts/audit_yolo_source_classes.py` reports 5,689 total labels with useful rare-class coverage (`20000-riel-b/f` = 190/205, `50000-riel-b/f` = 270/364). `scripts/check_duplicates.py --dataset cuurecy_detection_is --exact-only`, run through `scripts/run_with_headroom.py`, found 0 exact duplicate groups and 0 exact cross-split leakage groups across 2,329 images. `scripts/audit_yolo_segmentation_geometry.py`, also run through the headroom wrapper, found 5,689 valid segmentation rows, no malformed/out-of-bounds/zero-area polygons, and substantial edge-touching masks that support partial/off-frame coverage. Filtered `scripts/visual_audit_yolo_dataset.py` sheets show the tiny masks are mostly real far/partial/off-frame bills rather than broken labels, but hand occlusion, repeated layouts, near-duplicates, and note-version mix still need curation. Audit next for Roboflow license terms, banknote reproduction constraints, note version, near-duplicates, and whether masks are visually clean enough for the synthetic compositor. Also found [Currency Deteection](https://universe.roboflow.com/thesis-ivrzn/currency-deteection) with 4.7k broad Asian-currency detection images but only Cambodian riel 100/1000/5000 classes, and [ASEAN currency detection](https://universe.roboflow.com/mata-uang/currency-detection-y4tb2) with a single Cambodian Riel class. These are worth manual export/evaluation, but they do not replace the need for rights-clear Cambodian phone photos with reviewed partial/fan labels.
