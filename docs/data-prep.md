...
- `data/review/roboflow_cuurecy_detection_is_oldcommon_highconf_failure_review_v1/`: ignored historical review manifest generated from old/common classifier prediction CSVs; it records 31 high-confidence misses across `KHR_5000 -> KHR_10000`, `KHR_20000 -> KHR_10000`, and `KHR_20000 -> KHR_5000`, but its derived classifier crop paths are stale after cleanup.
- `data/review/cashsnap_p1_oldcommon_partial_focus_review_v1/`: ignored focused review queue generated by `scripts/build_partial_focus_review_queue.py`; current rebuild has 103 existing crops after recovering stale high-confidence rows via `source_crop`, with zero missing crop paths and front/back coverage for `KHR_5000`/`KHR_10000`/`KHR_20000`.
- `data/review/p1_focus_v2_oldcommon_failure_review_v1/`: ignored 43-row failure-review pack from the repaired P1 diagnostic predictions; it has zero missing crop paths and focuses on high-confidence `KHR_5000/20000 -> KHR_10000` plus `KHR_20000 -> KHR_5000` misses.
...
A later targeted sweep surfaced only adjacent leads, now logged in `manifests/blocked_or_manual_sources.csv`: a 2025 Kazakh banknote image dataset with mixed multi-note scenes but no boxes, and a 2025 USD missing-object dataset. Treat both as low-priority transfer/robustness material after KHR rights-clear thin-slice captures.
...
- `scripts/build_imagefolder_mix.py` can make diagnostic ImageFolder mixes from a base dataset plus train-only extras; use it for controlled probes without contaminating validation/test splits.
...
- `scripts/apply_review_export.py` merges `demo/review/` exports back into a source manifest by `crop_id` with `crop_path` fallback; use `--dry-run` first and avoid hand-editing long review CSVs.
- `scripts/build_partial_focus_review_queue.py --clean` regenerates the P1 old/common review queue from existing Roboflow packs; review this smaller queue before broader public-data curation.
