#!/usr/bin/env python
"""Train a small MobileNetV3 classifier from an ImageFolder dataset."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", required=True, type=Path, help="ImageFolder dataset with train/val folders.")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional existing classifier .pt checkpoint to fine-tune from.",
    )
    parser.add_argument(
        "--aug-profile",
        choices=["basic", "crop_transfer_v1", "denom_cue_v1"],
        default="basic",
    )
    ...
    parser.add_argument(
        "--freeze-features",
        action="store_true",
        help="Freeze MobileNet feature layers and train only the classifier head.",
    )
...
def build_transforms(image_size: int, aug_profile: str) -> tuple[transforms.Compose, transforms.Compose]:
    ...
    elif aug_profile == "crop_transfer_v1":
        train_steps = [
            ...
            transforms.RandomApply([transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 1.4))], p=0.25),
            transforms.ColorJitter(brightness=0.30, contrast=0.28, saturation=0.22, hue=0.04),
            transforms.RandomGrayscale(p=0.04),
            transforms.ToTensor(),
            normalize,
            transforms.RandomErasing(p=0.10, scale=(0.01, 0.05), ratio=(0.3, 3.3), value="random"),
        ]
    elif aug_profile == "denom_cue_v1":
        train_steps = [
            transforms.RandomResizedCrop(
                image_size,
                scale=(0.84, 1.00),
            ...
            transforms.RandomPerspective(distortion_scale=0.14, p=0.28),
            transforms.RandomRotation(
                degrees=6,
                interpolation=transforms.InterpolationMode.BILINEAR,
                fill=0,
            ),
            transforms.RandomApply([transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 1.1))], p=0.20),
            transforms.ColorJitter(brightness=0.26, contrast=0.24, saturation=0.20, hue=0.035),
            transforms.RandomGrayscale(p=0.03),
            transforms.ToTensor(),
            normalize,
        ]
    else:
        raise SystemExit(f"unknown augmentation profile: {aug_profile}")
...
def load_initial_checkpoint(model: nn.Module, checkpoint_path: Path, class_names: list[str], image_size: int) -> None:
    checkpoint = torch.load(resolve(checkpoint_path), map_location="cpu", weights_only=False)
    checkpoint_classes = [str(name) for name in checkpoint.get("classes", [])]
    if checkpoint_classes != class_names:
        raise SystemExit(f"init checkpoint classes differ: {checkpoint_classes} != {class_names}")
    checkpoint_image_size = int(checkpoint.get("image_size", image_size))
    if checkpoint_image_size != image_size:
        raise SystemExit(f"init checkpoint image_size {checkpoint_image_size} != requested {image_size}")
    model.load_state_dict(checkpoint["model_state"])


def freeze_feature_layers(model: nn.Module) -> None:
    features = getattr(model, "features", None)
    if features is None:
        raise SystemExit("model does not expose MobileNet feature layers to freeze")
...
def inverse_frequency_weights(counts: list[int]) -> torch.Tensor:
    ...
    return torch.tensor([total / max(1, class_count * count) for count in counts], dtype=torch.float32)
...
def main() -> None:
    ...
    train_tf, val_tf = build_transforms(args.image_size, args.aug_profile)
    ...
    if not class_names:
        raise SystemExit(f"No classes found under {repo_rel(data_dir)}")

    device = choose_device(args.device)
    model = build_model(len(class_names), args.pretrained)
    if args.init_checkpoint is not None:
        load_initial_checkpoint(model, args.init_checkpoint, class_names, args.image_size)
    ...
    if not trainable_parameters:
        raise SystemExit("no trainable model parameters")
            ...
            torch.save(
                {
                    ...
                    "init_checkpoint": repo_rel(resolve(args.init_checkpoint)) if args.init_checkpoint else "",
    ...
    (run_dir / "classes.json").write_text(json.dumps(class_names, indent=2) + "\n", encoding="utf-8")
    if args.export_onnx:
    ...
    print(f"saved: {repo_rel(best_path)}", flush=True)
